 ...
 * LOAD DATA
 * */
d3.csv("data-utlx4-clean.csv", d => ({
  period: new Set(d.Period),
  region: d.Area,
  revenue: +d.Revenue,
})).then(raw_data => {
...
function init() {
  ...
  xScale = d3
    .scaleTime()
    .domain(d3.extent(state.data, d => d.period))
    .range([margin.left, width - margin.right]);

  yScale = d3
    .scaleLinear()
    .domain([0, d3.max(state.data, d => d.revenue)])
...
function draw() {
  ...
  const dot = svg
    .selectAll(".dot")
    .data(filteredData, d => d.period) // use `d.year` as the `key` to match between HTML and data elements
    .join(
      enter =>
    ...
    .call(
      selection =>
        selection
          ...
          .attr("cy", d => yScale(d.revenue)) // started from the bottom, now we're here
    );
    ...
    .join(
      enter =>
        enter
          .append("path")
          .attr("class", "trend")
          .attr("opacity", 0.8), // start them off as opacity 0 and fade them in
